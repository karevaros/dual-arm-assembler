...
#
# 이 저장소를 ROS 2 워크스페이스의 src/ 아래에 두고 실행하세요:
#   ~/ros2_ws/src/dual-arm-assembler/scripts/setup_vendor_models.sh
#
...
#
# 사용: bash scripts/setup_vendor_models.sh
set -u
# 저장소 위치에서 워크스페이스를 유도한다(<ws>/src/<repo>/scripts/이파일).
HERE=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
REPO=$(dirname "$HERE")
SRC=$(dirname "$REPO")
WS=$(dirname "$SRC")
if [ "$(basename "$SRC")" != "src" ]; then
  echo "이 저장소는 ROS 2 워크스페이스의 src/ 아래에 있어야 합니다."
  echo "  현재 위치: $REPO"
  echo "  예)  mkdir -p ~/ros2_ws/src && mv $REPO ~/ros2_ws/src/"
  exit 1
fi
V=$SRC/vendor
...
echo
echo "완료. 확인: ros2 run dual_arm_assembler assembler"
...
#   RG2 는 이 저장소에 동봉된 모델(urdf/rg2)을 쓴다.
