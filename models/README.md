...
양팔은 **별도 패키지**로 분리돼 있다 — 기존 단팔 어셈블러는 그대로 두고, 양팔 전용 명령을 쓴다.

```bash
ros2 run rda_dual_assembler assembler      # 양팔(상체 + 팔·그리퍼 2조)
ros2 run rda_robot_assembler assembler     # 단팔(기존)
```

- 상체 모델은 이 폴더의 `torso/` 에 있다(`column_torso` 기둥형 440mm · `tbar_torso` T자형 600mm).
- 2번 팔/그리퍼는 **`arm/`·`endeffector/` 폴더를 그대로 공유**한다 — 팔 모델을 새로 넣으면
  1·2번 팔 드롭다운에 함께 나온다. `arm2/` 같은 폴더를 따로 만들 필요 없다.
- 자세한 절차·주의사항 → [`rda_dual_assembler/README.md`](../../../rda_dual_assembler/README.md)
