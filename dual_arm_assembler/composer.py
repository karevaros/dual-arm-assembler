...
슬롯별 `xacro:if` 분기를 손으로 관리하는 방식을 대체한다 — 분기를 빠뜨린 모델이 조용히
누락되는 문제(실제로 26종 중 21종이 하루 동안 빠져 있었다)의 근본 해결.
...
■ 링크명 계약(하위 산출물이 의존 — 바꾸지 말 것)
  모델 yaml 에 `prefix: ""` 를 준 모델은 원래 링크 이름을 그대로 유지한다(예: base_link ·
  link0 · tcp · rg2_hand). 하위 산출물(SRDF·기구학 스크립트 등)이 특정 이름에 묶여 있을 때
  쓰라고 남겨 둔 장치다. 그 외 모델에는 슬롯 접두사가 붙는다.
"""
import argparse
import copy
import os
...
def resolve_prefix(model, slot):
    ...
    if p is None:
        p = ""
    p = p.format(slot=slot)
    # ★ 2번 팔/그리퍼(arm2·endeffector2)에는 링크명 계약을 적용하지 않는다.
    #   rb5_850e·onrobot_rg2 는 link0·tcp·rg2_hand 를 보존하려고 prefix 가 "" 인데,
    #   같은 모델을 두 슬롯에 쓰면 이름이 그대로 충돌해 조립이 실패한다.
    #   1번 팔이 계약 이름을 갖고, 2번 팔은 슬롯 prefix(arm2_)로 분리한다.
    if not p and slot in getattr(reg, "NO_CONTRACT_SLOTS", ()):  # 예: arm2_ , endeffector2_
        p = f"{slot}_"
    return p
...
def compose(mounts_path, robot_name="assembled_robot"):
