...
from dual_arm_assembler import urdf_loader as ul
from dual_arm_assembler.assembly import Mount, compute_placements
from dual_arm_assembler import collision as col

# 앱 표시 이름. 실행 구성(단팔/양팔)에 따라 configs 가 덮어쓴다.
...
def default_mounts():
    """초안 기본 결합값(비교표 초안 기준). 등록된 슬롯 것만 남긴다."""
    d = {
    ...
    d.update(EXTRA_DEFAULT_MOUNTS)
    return {k: v for k, v in d.items() if k in reg.SLOTS}
...
class Assembler(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        # 양팔 슬롯은 기본 꺼짐 — 켜야 조립·저장에 들어간다(기존 단팔 구성 그대로 유지).
        self.enabled = {s: s not in getattr(reg, "OPTIONAL_SLOTS", ()) for s in reg.SLOTS}
    ...
    def _build_left(self):
        ...
        for slot in reg.SLOTS:
            ...
            # 선택 슬롯(상체·2번 팔/그리퍼)은 기본 꺼짐 — self.enabled 와 화면을 일치시킨다.
            chk.setChecked(bool(self.enabled.get(slot, True)))
    ...
    def _on_enable_changed(self, slot):
        ...
        self._sync_collider(slot)
        # ★ 슬롯을 새로 켜면 그 파트의 '설계상 겹침'을 기준에 넣어 준다.
        #   시작 시 켜져 있던 파트는 _calibrate_baseline(initial=True) 로 이미 받는 대우인데,
        #   나중에 켠 파트만 못 받으면 정상 결합(그리퍼가 플랜지에 얹힌 것 등)이 충돌로 뜬다.
    ...
    def _mounts_path(self):
        """조립 결과(mounts.yaml) 저장 위치.

        해석 순서: 환경변수 `RDA_MOUNTS_FILE` → 모델 폴더의 상위(=저장소 루트)/mounts.yaml.
        소스 위치에 저장해야 재빌드 없이 컴포저·launch 가 바로 읽는다."""
        p = os.environ.get("RDA_MOUNTS_FILE")
        if p:
            return os.path.expanduser(p)
        from dual_arm_assembler import paths as _paths
        return os.path.join(os.path.dirname(_paths.models_dir()), "mounts.yaml")
    ...
    def _load(self):
        ...
        # 선택 슬롯(torso·arm2·endeffector2)의 on/off 는 **파일에 그 슬롯이 있는지**로 복원한다.
        #   (저장은 켜진 슬롯만 기록하므로, 없으면 꺼져 있던 것이다.)
        for slot in getattr(reg, "OPTIONAL_SLOTS", ()):
            on = slot in mp and bool(self.models.get(slot))
            self.enabled[slot] = on
            w = self.slot_widgets.get(slot, {}).get("chk")
            if w is not None:
